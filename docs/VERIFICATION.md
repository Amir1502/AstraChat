...
- По-прежнему не проверено: установка APK на реальное устройство, ручное визуальное QA, TalkBack, ландшафт, большие экраны и реальные запросы к API провайдеров. Job `release` с 1.0.1 не зависит от `device-tests`; теперь, когда они зелёные, этот гейт можно вернуть.
