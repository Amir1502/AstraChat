...
- Изменения в коде: `data/SettingsStore.kt` (новое поле `allowScreenshots: Boolean = true` в сериализуемых `AppSettings` — старые сохранённые настройки читаются, недостающее поле получает значение по умолчанию), `MainActivity.kt` (старт всегда с `FLAG_SECURE`, чтобы окно было защищено до загрузки настроек), `ui/AstraRoot.kt` (`ApplyScreenshotPolicy` применяет сохранённое значение к `LocalActivity.current?.window`), `ui/SettingsScreen.kt` (раздел «Приватность экрана» с переключателем и пояснением о последствиях). Значение по умолчанию — `true` по решению владельца: скриншоты разрешены, защита окна становится опцией. В первой редакции правки дефолт был `false`, но ни одна опубликованная сборка с ним не выходила.
...
- Факты v1.0.0 проверены скачиванием: `AstraChat-v1.0.0-debug-universal.apk` — 14 062 554 байт, SHA-256 `6365784f8f0b4c505101ae03a8557575c4d279f9f735119497128b8dbebd3f57`, `apksigner verify --print-certs` → сертификат `C=US, O=Android, CN=Android Debug`, SHA-256 сертификата `3a93de1f1348a879fefae0f65d5ce955a7aafc537a495ff208d86036176d82f9`, `aapt2 dump badging` → versionCode 1, versionName 1.0.0, minSdk 26.

## Дополнение — релиз 1.0.3 (скриншоты разрешены по умолчанию)

- Тег `v1.0.3` на коммите `486d57b`. CI-прогон [34232076292](https://github.com/Amir1502/AstraChat/actions/runs/34232076292): `verify`, `device-tests (26)`, `device-tests (35)` и `release` — все успешны; релиз опубликован только после двух зелёных гейтов.
- Политика исходников на чистом worktree HEAD: `python tools/check_source.py` → PASS, 62 файла (локально в рабочей копии скрипт падает только на неподписанных APK в неотслеживаемом `dist/`, которых в checkout нет).
- Скачанный ассет `AstraChat-v1.0.3-universal.apk`: 1 981 711 байт, SHA-256 `b5d358439e47c5d9609be27f7c551a265e17a9ca174cfcb63400a17113df388a` — совпадает со строкой в `SHA256SUMS.txt` из того же релиза.
- `apksigner verify --verbose --print-certs`: Verifies; v1 (JAR) = false, v2 = true, v3/v3.1/v4 = false; сертификат `CN=Astra Chat Release, OU=Mobile, O=AstraChat, C=US`, SHA-256 сертификата `e0ce85d9195b6fde6679653ac46d864dadaa20666d5f63973c7a07166bd06a25`, SHA-256 публичного ключа `417a7e2715e32d2fb578d8ce7a676f5dd2b9a561eb3fa341a12aa944fd5ab9e6`. Ключ тот же, что у 1.0.1 и 1.0.2, поэтому обновление устанавливается поверх без удаления.
- `aapt2 dump badging`: package `com.folzi.astrachat`, versionCode 4, versionName 1.0.3, minSdkVersion 26, targetSdkVersion 37, compileSdkVersion 37, native-code arm64-v8a / armeabi-v7a / x86 / x86_64.
- Описания релизов приведены к одному шаблону (таблица ассетов с размером и SHA-256, данные подписи, шаги установки и обновления, блоки «Проверено» и «Не проверено»): v1.0.3 — id 384762173, 4241 символ; v1.0.2 — 3985; v1.0.1 — 3210; v1.0.0 — 1593 с `prerelease = true` и без unsigned-ассета. `releases/latest` теперь указывает на v1.0.3.
- Не проверено: установка на реальное устройство, фактическое поведение `FLAG_SECURE` и полученных снимков экрана, визуальное QA, TalkBack, ландшафт и планшет, длинные истории, реальные запросы к платным API. Скриншоты в репозитории отсутствуют — публиковать нарисованные макеты намеренно не стали.
