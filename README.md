...
**Данные и статистика**
- Room: чаты, сообщения, ветки, провайдеры, модели, usage, черновики, MCP-серверы; миграции 1→2, 2→3, 3→4 и 4→5 без destructive fallback.
...
**Вложения**
- Изображения (PNG/JPEG/WebP/GIF, до 4 МБ) и текстовые файлы или код (до 200 КБ) через системный выбор документов (SAF) — до 5 файлов на сообщение, без разрешений на хранилище.
- Изображения отправляются модели во всех четырёх протоколах (Chat Completions, Responses, Anthropic, Gemini), текстовые файлы — содержимым в сообщении; байты изображений хранятся в приватном каталоге приложения и не попадают ни в историю, ни в бэкапы (там остаются имя, тип, размер и текст).
...
| Тесты | Состав |
|---|---|
| Unit (JVM), 65 | `McpClientTest` — 24, `NetworkTest` — 9, `ProviderCodecTest` — 21, `AstraViewModelTest` — 11 |
| Инструментальные, 8 | `StorageTest` — 6 (миграции Room 1→2, 2→3, 3→4, 4→5, ветвление и бэкапы, шифр SecretVault); `ComposeSmokeTest` — 2 (диалог подтверждения, рендер UI-примитивов) |
...
- Импорт/экспорт JSON переносит чаты, сообщения и ветки, но не ключи, настройки, черновики и биллинговую статистику. Голосового ввода нет. Из вложений в копию попадают текст файлов и метаданные; байты изображений остаются в приватном каталоге приложения — после восстановления на другом устройстве изображение деградирует до текстовой отметки.
...
| Что | Как проверено |
|---|---|
| Сборка и статический анализ | Локально: detekt, 65 unit-тестов, Android Lint (5 прежних замечаний, ноль новых), `assembleDebug`, компиляция androidTest — зелёные; `verify` в CI повторяет тот же набор |
| Поведение на эмуляторе | `device-tests` API 26 и 35 в CI: миграции Room 1→2, 2→3, 3→4, 4→5, ветвление и бэкапы, SecretVault, Compose-тесты. Локально AVD нет — инструментальные тесты выполняются только в CI; миграции 2→3, 3→4 и 4→5 ещё не прогонялись на эмуляторах |
...
Сделано: воспроизводимая зелёная сборка в CI, подписанные релизы, инструментальные тесты на эмуляторах API 26 и 35, редизайн интерфейса, настраиваемая защита окна (скриншоты разрешены по умолчанию), tool calls для моделей через подключённые MCP-серверы (Chat Completions, этап 2), вложения в чат — изображения и текстовые файлы (1.1.0).

Дальше: полное ручное QA на устройстве (установка, снимки экрана, TalkBack, ландшафт, планшет, длинные истории) и публикация скриншотов, бенчмарк большой истории, зашифрованное резервное копирование, пагинация model discovery у разных провайдеров, Responses/Anthropic/Gemini tool calling (этап 3). Эти задачи не выдаются за выполненные.
