...
1. Скачайте `AstraChat-v1.0.2-universal.apk` со страницы [релиза v1.0.2](https://github.com/Amir1502/AstraChat/releases/latest) (1 981 711 байт).
2. Сверьте хеш с `SHA256SUMS.txt` из того же релиза:
   ```powershell
   # Windows PowerShell
   Get-FileHash AstraChat-v1.0.2-universal.apk -Algorithm SHA256
   ```
   ```bash
   # Linux / macOS
   sha256sum AstraChat-v1.0.2-universal.apk
   ```
   Ожидается `36a4c9fdc02dc1dbb75048f4f4ff98cebd3dc9d8711497226d7d6e31a141b4f9`.
...
| 1.0.1 → 1.0.2 | ключ один и тот же | ставится поверх, удаление не нужно |
| «Неизвестный источник» | политика Android для файлов из браузера | разрешите установку из браузера или файлового менеджера |

Проверить подпись установленного файла можно и на компьютере: `apksigner verify --print-certs AstraChat-v1.0.2-universal.apk` (build-tools Android SDK).
...
- Защита окна флагом `FLAG_SECURE` по умолчанию и переключатель, разрешающий скриншоты (см. [Скриншоты](#screens)).

<a id="design"></a>
## Дизайн

Интерфейс 1.0.2 — нейтральный минимализм в духе инструментов для кода: плоские графитовые поверхности, hairline-рамки 1 dp вместо теней, один приглушённый синий акцент, никакого «пузырного» чата.
...
- В релизных APK **1.0.2 и старше** окно постоянно защищено `FLAG_SECURE`: система не даёт снять экран и не показывает содержимое в недавних приложениях.
- В текущей ветке `main` защита стала настройкой: **Настройки → Приватность экрана → «Разрешить скриншоты и превью в недавних»**. По умолчанию выключено, применяется сразу без перезапуска.
- Чтобы добавить снимки в этот раздел: включите настройку, сделайте кадры на устройстве, положите PNG в `docs/screenshots/` и вставьте ссылки сюда. Для публикации выбирайте debug-сборку и пустые чаты — не выкладывайте настоящую переписку и ключи.
...
- Окно по умолчанию под `FLAG_SECURE`: системные снимки и миниатюры в недавних приложениях блокируются. Отключается только явным переключателем в настройках.
...
Сделано: воспроизводимая зелёная сборка в CI, подписанные релизы, инструментальные тесты на эмуляторах API 26 и 35, редизайн интерфейса.

Дальше: полное ручное QA (TalkBack, ландшафт, планшет, длинные истории), бенчмарк большой истории, зашифрованное резервное копирование, пагинация model discovery у разных провайдеров, вложения и tool calls. Эти задачи не выдаются за выполненные.
