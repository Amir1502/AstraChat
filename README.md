...
**Данные и статистика**
- Room: чаты, сообщения, ветки, провайдеры, модели, usage, черновики, MCP-серверы; миграции 1→2, 2→3 и 3→4 без destructive fallback.
...
| Тесты | Состав |
|---|---|
| Unit (JVM), 57 | `McpClientTest` — 24, `NetworkTest` — 9, `ProviderCodecTest` — 15, `AstraViewModelTest` — 9 |
| Инструментальные, 7 | `StorageTest` — 5 (миграции Room 1→2, 2→3, 3→4, ветвление и бэкапы, шифр SecretVault); `ComposeSmokeTest` — 2 (диалог подтверждения, рендер UI-примитивов) |
...
- Импорт/экспорт JSON переносит чаты, сообщения и ветки, но не ключи, настройки, черновики и биллинговую статистику. Вложений и голоса нет.
...
| Что | Как проверено |
|---|---|
| Сборка и статический анализ | Локально: detekt, 57 unit-тестов, Android Lint (5 прежних замечаний, ноль новых), `assembleDebug`, компиляция androidTest — зелёные; `verify` в CI повторяет тот же набор |
| Поведение на эмуляторе | `device-tests` API 26 и 35 в CI: миграции Room 1→2, 2→3, 3→4, ветвление и бэкапы, SecretVault, Compose-тесты. Локально AVD нет — инструментальные тесты выполняются только в CI; миграции 2→3 и 3→4 ещё не прогонялись на эмуляторах |
...
Сделано: воспроизводимая зелёная сборка в CI, подписанные релизы, инструментальные тесты на эмуляторах API 26 и 35, редизайн интерфейса, настраиваемая защита окна (скриншоты разрешены по умолчанию), tool calls для моделей через подключённые MCP-серверы (Chat Completions, этап 2).

Дальше: полное ручное QA на устройстве (установка, снимки экрана, TalkBack, ландшафт, планшет, длинные истории) и публикация скриншотов, бенчмарк большой истории, зашифрованное резервное копирование, пагинация model discovery у разных провайдеров, вложения, Responses/Anthropic/Gemini tool calling (этап 3). Эти задачи не выдаются за выполненные.
