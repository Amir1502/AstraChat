...
- Job `release` больше не зависит от `device-tests`: на момент 1.0.1 эмуляторные прогоны (API 26/35) падали на hosted-раннерах и не должны были блокировать публикацию собранных APK. Причина падений устранена в разделе Unreleased — теперь `device-tests` зелёные, и гейт можно вернуть.
