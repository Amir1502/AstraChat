    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Настройки") }, navigationIcon = { Action("Назад", onClick = back) }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).verticalScroll(rememberScrollState()).padding(16.dp).widthIn(max = 760.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            ...
            Section("Приватность экрана") {
                ...
                Text(
                    "Скриншоты разрешены: снимки экрана и миниатюры в недавних приложениях работают как обычно. Выключите переключатель, чтобы вернуть защиту Android FLAG_SECURE — тогда система перестанет снимать это окно и скроет его содержимое в недавних приложениях. Готовые снимки попадают в галерею и могут быть доступны другим приложениям, поэтому не публикуйте переписку и ключи. Настройка применяется сразу, без перезапуска.",
